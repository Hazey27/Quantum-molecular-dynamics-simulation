# Hazel Dellario
# 2024.03.24
# Modeling H2 dynamics by using Morse potential to get force and reframing problem as QUBO and then QAOA problem
...
from qiskit.primitives import Sampler  # QAOA(sampler=Sampler(), optimizer=COBYLA(), initial_point=[0.3,0.1])

import test_core
import paired_down_qde as qde


diffeq = QuadraticProgram()
...
def get_problem(problem_id, p, r, **kwargs):  # I CHANGED TO EXPLICILTLY REQUIRING P AND R
    ...
    system_terms[0] = p / Hydrogen.mu
    system_terms[1] = Hydrogen.get_force_morse(r)

    boundary_condition = np.array([initial_position, 0])
    solution = lambda t: Hydrogen.morse_trajectory_v0(initial_position, t)  # we're not scaling momentum in this house (at least not at this stage)
